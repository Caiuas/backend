...
def enviar_pesquisas():
    """Função principal que executa o envio das pesquisas"""
    query = f"""
            SELECT
            TO_CHAR(
                CASE
    ...
    rows = cur.fetchall()
    cont = 0
    print(f"[{datetime.now()}] Total de eventos encontrados: {len(rows)}")
        ...
        if len(telefones_elegiveis) > 0:
            ...
            for telefone in telefones_elegiveis:
                ...
                try:
                    ...
                    erros += 1
                    print(f"[{datetime.now()}] Erro ao enviar para {telefone}: {e}")
    ...
    cur.close()
    conn.close()
    print(f"[{datetime.now()}] Processamento concluído. Total de eventos processados: {cont}")

def main():
    """Loop principal que verifica o horário e executa às 12:00"""
    target_time = time(16, 31)  # 12:00
    executed_today = False
    
    print(f"[{datetime.now()}] Script iniciado. Aguardando 12:00 para executar...")
    
    while True:
        current_time = datetime.now().time()
        current_date = datetime.now().date()
        
        # Se passou das 12:00 e ainda não executou hoje
        if current_time >= target_time and not executed_today:
            print(f"[{datetime.now()}] Iniciando envio de pesquisas...")
            try:
                enviar_pesquisas()
                executed_today = True
                print(f"[{datetime.now()}] Execução concluída com sucesso!")
            except Exception as e:
                print(f"[{datetime.now()}] Erro durante a execução: {e}")
        
        # Reset flag no próximo dia
