...
WHERE 1=1
	AND TRUNC(os.DATA_EMISSAO) >= TO_DATE('{initial_date.strftime("%Y-%m-%d")}', 'YYYY-MM-DD')
	AND TRUNC(os.DATA_EMISSAO) <= TO_DATE('{now.strftime("%Y-%m-%d")}', 'YYYY-MM-DD')
	AND os.STATUS_OS <> 2
	AND os.numero_os < 0
GROUP BY
	e.NOME,
...
df_email = df.copy()
# padroniza Nome/Email a partir das colunas da consulta de orcamentos
...
if access_token:
    for _, linha in df_email.iterrows():
        ...
        payload = {
            ...
            "event_family": "CDP",
            "payload": {
                "conversion_identifier": "orcamentos",
                "name": nome,
                "email": email,
                "company_name": nome,
                ...
                "tags": ["orcamentos", "NBS", modelo_carro],
                "traffic_source": "NBS",
...
message = f"Relatorio Orcamentos - {now.strftime('%d/%m/%Y')}"
files = {
    "document": ("relatorio_orcamentos.xlsx", planilha.getvalue())
}
data = {
